...
- Reads `maxconversionsperscan` environment variable (positive integer, default: `20`): maximum number of songs to convert per scan before sleeping again.
...
- Converts existing `.m4a` files at startup in batches so large backlogs are processed without waiting a full poll interval between every file.
- Replaces each source file with `<same-name>.flac`.
- Continues watching for new/updated `.m4a` files, waits one poll interval for files to settle, and then converts up to the configured batch size per scan.
...
```bash
TZ=America/New_York PUID=1000 PGID=1000 path=/absolute/path/to/music subfolder=true polldelayseconds=60 maxconversionsperscan=20 python app.py
```
